...
An ADR is written *after* the decision, by whoever made it, and is never
retroactively edited to match new reality. When a decision changes, a new ADR
supersedes the old one and the old one stays, marked superseded. That is the
...
- Written in past tense about a decision that was made, not a proposal.
- Cite the code that implements it, so a reader can check the ADR is still true.
- Never edit an accepted ADR to match changed code. Supersede it.
...
| [0003](0003-blocker-title-cannot-contain-source-title.md) | A blocker's title cannot contain the source event's title, by signature | accepted |
...
| [0008](0008-drag-validated-against-rendered-order.md) | A drag is validated against the order it was drawn on | accepted |
| [0009](0009-source-order-is-load-bearing.md) | Source order is load-bearing | accepted |
| [0010](0010-source-id-in-marker-orphans-on-rename.md) | The source id is embedded in every marker, so renaming orphans events | accepted |
