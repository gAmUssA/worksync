...
- Per-source form covers every source field: `id`, `account`, `calendar`, `title_template`, `target_calendar`, `availability`, the `coalesce` / `include_all_day` / `skip_if_work_busy` toggles, the `title_matches` / `title_excludes` lists, and the minute knobs (`coalesce_gap_minutes`, `min_duration_minutes`, `padding_before_minutes`, `padding_after_minutes`).
