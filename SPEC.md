...
- Per-source form covers every source field: `id`, `account`, `calendar`, `target_calendar`, `title_template`, `availability`, the `coalesce` / `include_all_day` / `skip_if_work_busy` toggles, the minute knobs (`coalesce_gap_minutes`, `min_duration_minutes`, `max_duration_minutes`, `padding_before_minutes`, `padding_after_minutes`), `skip_weekdays`, and `title_matches` / `title_excludes`. `target_calendar` is a popup over the target account's writable calendars with an explicit row for the empty default, never free text, for the same reason the account and calendar pickers are popups. `max_duration_minutes` renders `0` as "no limit" rather than as zero minutes, and a maximum below the minimum is refused at the field; `coalesce_gap_minutes` is disabled while `coalesce` is off, since that is when it does nothing; the seventh `skip_weekdays` switch will not turn on, because skipping every day mirrors nothing. Duration-window and all-seven-days refusals are backed by `ConfigLoader.validate`. Disabling the coalescing gap is UI-only applicability guidance, not a validation error. Calendar-title ambiguity is checked against all loaded calendars in the account before save, using `Resolver`’s name comparison regardless of writability; `Resolver` enforces ambiguity again at sync time. Config validation does not have calendar enumeration and does not check ambiguity. The two list-valued fields get a row per entry rather than one comma-joined field, since a calendar title can contain a comma; a blank or duplicate entry is refused at the field, because `ConfigLoader.validate` throws on a blank one and that error names a config key far from the keystroke that caused it. The form states that a title is read only to decide what to mirror, which is the one place §7's privacy invariant looks violated at a glance.
