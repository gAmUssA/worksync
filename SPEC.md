...
- Per-source form covers every source field: `id`, `account`, `calendar`, `title_template`, `target_calendar`, `availability`, the `coalesce` / `include_all_day` / `skip_if_work_busy` toggles, the minute knobs (`coalesce_gap_minutes`, `min_duration_minutes`, `padding_before_minutes`, `padding_after_minutes`), and `title_matches` / `title_excludes`. The two list-valued fields get a row per entry rather than one comma-joined field, since a calendar title can contain a comma; a blank or duplicate entry is refused at the field, because `ConfigLoader.validate` throws on a blank one and that error names a config key far from the keystroke that caused it. The form states that a title is read only to decide what to mirror, which is the one place §7's privacy invariant looks violated at a glance.
