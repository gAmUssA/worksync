...
- Per-source form covers every source field: `id`, `account`, `calendar`, `title_template`, `target_calendar`, `availability`, the `coalesce` / `include_all_day` / `skip_if_work_busy` toggles, and the minute knobs (`coalesce_gap_minutes`, `min_duration_minutes`, `padding_before_minutes`, `padding_after_minutes`). `title_matches` / `title_excludes` are config-file-only for now: they are list-valued, so they need a list editor rather than another row, and that is tracked separately.
