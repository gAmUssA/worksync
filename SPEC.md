...
- Exit codes: 0 success, 1 config/validation error, 2 permission error (with remediation text), 3 partial failure (some writes failed or a backend call failed; safe to re-run). The mapping is a pure function in the core (`ExitCodes.code(for:)`) so it stays unit-testable and no error path can silently bypass it — an uncaught error reaching the argument parser would collapse every failure onto one status. Anything unrecognized maps to 3, never to 1: reporting a runtime failure as a config error sends the user to edit a file that is not the problem.
