...
- **No windowed GUI for sync operation, and no cloud- or network-backed configuration.** Config remains a plain TOML file on disk and must stay hand-editable — this constraint is what forces any editor UI to preserve comments/formatting rather than becoming the only supported path. A menu bar status item (§11) and the config editor window it can open (§11.1) are the only UI.
...
5. **SwiftUI hosting:** the Preferences window is SwiftUI hosted via `NSHostingController` inside a plain `NSWindow` (not the SwiftUI App/Settings scene — that conflicts with the NSApplication + app-delegate lifecycle this process owns). The NSWindow's `styleMask` MUST include `.fullSizeContentView`: without it, `NavigationSplitView`'s sidebar cannot extend to full height and the sidebar toggle misbehaves — this window-configuration flag, not the hosting mode itself, is the root cause of the "dead sidebar toggle" symptom. Drive sidebar collapse through a bound `NavigationSplitViewVisibility` (`columnVisibility`) rather than the AppKit `toggleSidebar` selector hop. If the system toolbar toggle still misbehaves with the correct styleMask, fall back to a custom toolbar button flipping the binding, and verify against the running app.
6. **General principle:** when a framework feature "does nothing" or crashes for no apparent reason, verify the packaging first (signature, LaunchServices registration, launch path, styleMask) — most "broken framework" symptoms in this architecture are assembly mistakes. Prefer explicit, state-driven implementations over behaviors that depend on a scene lifecycle, and verify UI behavior against the actually running app rather than assuming a documented API works here.
...
- Exit codes: 0 success, 1 config/validation error, 2 permission error (with remediation text), 3 partial failure (some writes failed; safe to re-run).
...
- Concurrent runs: acquire an exclusive flock on `~/.config/worksync/.lock`; if held, exit 0 quietly (another run in progress).
...
NSStatusItem in the system menu bar, AppKit directly (no SwiftUI requirement). Accessory behavior (no Dock icon) comes from `LSUIElement = true` in the bundle's Info.plist (§3) — no unconditional `setActivationPolicy` call at startup.
...
- Header line: `Last sync: 14:32 — created 1, deleted 2` (or the error message)
- Per-source status lines: `personal: 12 blocks`, `travel: 3 blocks`
- **Sync now** — triggers an immediate pass (disabled while one is running)
- **Pause syncing / Resume** — toggle; paused state survives restarts (persist in UserDefaults)
- **Launch at login** — SMAppService toggle (§10)
- **Preferences…** — opens the config editor window (§11.1)
- **Open config** — opens config.toml in the default editor and then validates it, surfacing any config error in an NSAlert immediately rather than letting a bad edit fail invisibly at the next scheduled pass. Config is re-read at the start of every pass, so edits apply on the next sync without a restart. This raw-file path always remains available alongside the editor window.
- **Open log**
- **Quit**
...
### 11.1 Config editor window

A Preferences window that edits config.toml through the same on-disk file the CLI reads — config.toml stays the single source of truth and is never replaced by a UI-owned store.

- SwiftUI hosted in `NSHostingController` inside a plain `NSWindow` with `.fullSizeContentView` in its styleMask, opened from the menu. Not the SwiftUI App/Settings scene (§3.1 rule 5). Because the app is an LSUIElement accessory, opening the window must call `NSApp.activate(ignoringOtherApps: true)` alongside `makeKeyAndOrderFront`, or it will not come forward; if that proves unreliable, temporarily flip the activation policy to `.regular` while the window is open and back to `.accessory` on close (§3, the one sanctioned use of the runtime activation-policy API).
- Layout: a sidebar listing "General" plus one row per source, and a detail form for the selection.
- The real value of this window is that account and calendar are resolver-backed popups populated from the same account/calendar enumeration `worksync calendars` uses — for `[target]` and for each source. A free-text typo in an account or calendar title hard-errors the entire sync (§4.1); a popup cannot be wrong.
...
- Sidebar collapse is driven through a bound `columnVisibility` (`NavigationSplitViewVisibility`). With `.fullSizeContentView` set on the window (§3.1 rule 5) the system sidebar toggle is expected to work; if it still misbehaves in practice, replace it with a custom toolbar button flipping the binding. Verify against the running app either way.
...
- If config.toml does not currently parse, Preferences refuses to open and points the user at "Open config" instead, rather than presenting a form full of defaults that would overwrite the broken file with garbage on save.
...
- Manual test checklist in README: recurring source event moved (both a detached single occurrence and the whole series), source event deleted, source event toggled busy → free (blocker must disappear) and back, flight with padding crossing midnight, coalesced block gaining/losing a constituent, purge, TCC denial path, notification authorization prompt appears on first enable and a banner is delivered, launch-at-login toggle round-trips through System Settings, menubar pause/resume, wake-from-sleep sync, Sync now clicked repeatedly (must run a full pass every time, not just the first), Preferences round-tripping an edit into config.toml with all comments intact.
...
6. **M6:** config editor window (§11.1) — validate-and-surface on Open config, the config writer with backup and round-trip self-check (§4.3), the comment-preserving line editor, the general/target forms, and the source list with add/remove/drag-reorder and the id-rename warning.
...
- Editing a value in Preferences and saving updates exactly that value in config.toml, leaves every other line byte-identical, and preserves all comments; a save with no edits leaves the file byte-identical.
- Reordering sources in Preferences changes their order in config.toml, and renaming an existing source's id prompts a warning first.
