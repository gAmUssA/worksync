...
title_matches = []                 # [] = every title; else mirror only titles
                                   # containing one of these substrings
title_excludes = []                # titles containing one of these are never
                                   # mirrored; applied after title_matches
...
- A source's `id` is embedded verbatim in every managed event's marker (§7). Renaming an id strands the events created under the old one: the new id never matches them, so reconciliation treats them as unmatched managed events and schedules them for deletion — but only the ones it fetches, which is the current sync window on the currently resolved target calendars (§6). Events beyond that window, or on a calendar the source no longer targets, are never seen and remain; `worksync purge --source <old-id>` is how they are reached. This is a data-integrity concern, not a UI nicety: any tool that writes config.toml must warn before committing an id rename on an existing source, and must tell the user the purge command that reaches the stranded events. Ids of brand-new sources need no warning, since nothing exists under them yet.
...
- `title_matches` / `title_excludes` gate on the source event's title, so a source can mirror one class of event off a calendar carrying many — the "Personal Commitment" holds another sync tool writes onto a work calendar, where the real meetings already live on the work calendar itself. Non-empty `title_matches` requires a title to contain one of its substrings; `title_excludes` drops a title containing one of its substrings and is applied second, so an event hitting both lists is dropped. Matching is case- and diacritic-insensitive, and substring rather than equality, because calendar titles routinely carry suffixes a user will not think to type. **The title is read to decide eligibility and is never copied onto the blocker** — the blocker's title still comes from `title_template`, so §7's privacy invariant is untouched; this is the only place in the pipeline where a source title is read at all, and it reaches no field of the created event. An empty-or-blank entry is a config error rather than a no-op: `""` matches every title and would silently disable the filter the user thought they wrote.
- Every filter runs in the step-3 eligibility pass, which puts them before padding, before coalescing, and before cross-source dedup — so a filtered-out event never claims an identity and never blocks a later source from mirroring it (§5 step 5).
...
- The source id is embedded in the marker verbatim, which is what makes renaming a source id in config a stranding operation (§4.1). Reconciliation deletes the old-id events it fetches — the sync window on the currently resolved target calendars — and a purge filtered by source id (§8) is how the ones outside that scope are reached afterwards.
...
- The real value of this screen is that account and calendar are resolver-backed popups populated from the same account/calendar enumeration `worksync calendars` uses — for `[target]` and for each source. A free-text typo in an account or calendar title hard-errors the entire sync (§4.1); the pickers exclude titles ambiguous under the resolver’s comparison across all calendars in the selected account, before limiting target choices to writable calendars. Enumeration can become stale; sync still resolves the configured names.
...
- Per-source form covers every source field: `id`, `account`, `calendar`, `target_calendar`, `title_template`, `availability`, the `coalesce` / `include_all_day` / `skip_if_work_busy` toggles, the minute knobs (`coalesce_gap_minutes`, `min_duration_minutes`, `max_duration_minutes`, `padding_before_minutes`, `padding_after_minutes`), `skip_weekdays`, and `title_matches` / `title_excludes`. `target_calendar` is a popup over the target account's writable calendars with an explicit row for the empty default, never free text, for the same reason the account and calendar pickers are popups. `max_duration_minutes` renders `0` as "no limit" rather than as zero minutes, and a maximum below the minimum is refused at the field (no timed event can pass those bounds; all-day events bypass duration filtering); `coalesce_gap_minutes` is disabled while `coalesce` is off, since that is when it does nothing; the seventh `skip_weekdays` switch will not turn on, because skipping every day mirrors nothing. Duration-window and all-seven-days refusals are backed by `ConfigLoader.validate`. Disabling the coalescing gap is UI-only applicability guidance, not a validation error. Calendar-title ambiguity is checked against all loaded calendars in the account before save, using `Resolver`’s name comparison regardless of writability; `Resolver` enforces ambiguity again at sync time. Per-source target choices (including inheritance) and Save also use the resolver’s feedback-loop guard; a target cannot resolve to any source calendar. Case-only differences in saved account/calendar spelling select the existing picker row. New sources seed from unambiguous choices. Config validation does not have calendar enumeration and does not check ambiguity. The two list-valued fields get a row per entry rather than one comma-joined field, since a calendar title can contain a comma; a blank or duplicate entry is refused at the field, because `ConfigLoader.validate` throws on a blank one and that error names a config key far from the keystroke that caused it. The form states that a title is read only to decide what to mirror, which is the one place §7's privacy invariant looks violated at a glance.
...
- Renaming an existing source's id must raise a confirmation alert explaining that every managed event's marker embeds the source id, that the next sync deletes and recreates the ones it fetches, that events outside the sync window or off the source's current target calendars are left behind, and that `worksync purge --source <old-id>` is how those are reached (§4.1, §7). New sources are exempt.
...
- **The fast path is best-effort, never a guarantee.** It was verified working on macOS 26.6.1 during M7: an edit made in another process was observed firing `EKEventStoreChanged`, debounced, and driving a pass that wrote (`created=1`), with the resulting own-write echo correctly suppressed — and the same for the matching delete. This supersedes an earlier note recorded on macOS 26.5.2, where the notification was never observed firing under any observer configuration; that was a local observation rather than established platform fact, and the registration pitfalls above are the likelier explanation. **The design conclusion is unchanged and still binding:** App Nap can defer delivery to an accessory app, nothing is delivered while the machine sleeps, and the interval timer plus the wake-from-sleep pass are therefore not optional — they are the safety net that makes shipping this fast path acceptable. When the notification does not arrive, behavior degrades silently to timer-only polling, which is the correct and expected fallback. Do not build any feature that assumes the notification will arrive.
...
- The menu bar model is tested against the real `MenuBarModel`, not a copy of it. `MenuBarServices` injects every effect it has on the world — config load/save, persistence, logging, the login item, health, calendar choices, running a pass, notifications, change observation, the clock and the debounce — and `MenuBarModel.live(configPath:)` is the one composition that supplies the real ones. The designated initializer calls none of them, so a test can build a model without prompting for calendar access or touching the user's config; `IsolationTests` asserts that rather than assuming it. Editing state that once had a hand-written mirror in the Core tests — selection, drafts, row identity, renames, saving, reordering — is exercised through the model that ships, because a mirror keeps passing while the real thing drifts.
...
- With `change_driven = true`, an edit made in another process triggers a debounced pass; the write that pass performs does not trigger a further pass (own-write echo suppressed), and a burst of notifications for one edit collapses into a single pass.
- With `change_driven = true` and the notification never arriving, the interval timer and wake pass still run every pass on schedule — the fast path is additive and never load-bearing.
