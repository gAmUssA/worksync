...
- Per-source form covers `id`, `account`, `calendar`, `title_template`, `availability`, the `coalesce` / `include_all_day` / `skip_if_work_busy` toggles, the minute knobs it exposes (`min_duration_minutes`, `padding_before_minutes`, `padding_after_minutes`), and `title_matches` / `title_excludes`. Four source fields have no control yet and are config-file-only: `target_calendar`, `coalesce_gap_minutes`, `max_duration_minutes`, `skip_weekdays` — tracked separately, and the list is stated here rather than claimed away, because a form that silently omits a field is indistinguishable from one that has none. The two list-valued fields get a row per entry rather than one comma-joined field, since a calendar title can contain a comma; a blank or duplicate entry is refused at the field, because `ConfigLoader.validate` throws on a blank one and that error names a config key far from the keystroke that caused it. The form states that a title is read only to decide what to mirror, which is the one place §7's privacy invariant looks violated at a glance.
