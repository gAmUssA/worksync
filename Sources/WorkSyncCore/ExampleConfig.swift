...
public enum ExampleConfig {
    ...
    #
    # RENAMING THIS STRANDS EVENTS CREATED UNDER THE OLD NAME. The next sync no
    # longer recognizes them: it deletes the ones it fetches — inside the sync
    # window, on a calendar this source still writes to — and creates them again
    # under the new name. Any beyond that window, or on a calendar this source no
    # longer targets, are left behind; `worksync purge --source <old-id>` is how you
    # reach those. Choose it once.
    #
